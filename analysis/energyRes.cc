...
int main(int argc,char** argv){
    ...
    for(int ievt(0); ievt<data->GetEntries(); ievt++){
        ...
        if(recoTracks.size() == 1)
                h_trueEvsRng->Fill((*recoTracks.begin())->GetRecoRange(),(*trackIdToGunEnergy.begin()).second);
        ...
        //if(recoTracks.size() == 1)event->DrawSciDetHits(config);
        // event->DrawCMOSHits(config);
    }
...
//+0.5*(maxEdep[prelayerID]-maxEdep[prelayerID-1])/(maxEdep[prelayerID]+maxEdep[prelayerID-1])
        ...
        double recoEnergyByRng = fitval->Eval((*recoTracks.begin())->GetRecoRange());
        ...
        //if(trueEnergy<80) cout << "Bin/True/Reco/RngRes: " << (*recoTracks.begin())->GetRecoRange()+(*recoTracks.begin())->GetRngCorr() << "," << trueEnergy << "," << recoEnergyByRng << "," << RngRes << endl;
    }
