...
{  
    ...
    for (int ind=0; ind<pCTXMLInput->GetNProtons(); ind++){
        if(pCTXMLInput->GetBeamType() == "Rectangular"){
            ...
            G4double x0 =   (G4UniformRand()-0.5)*(pCTXMLInput->GetPlaneColumns()*pCTXMLInput->GetPixelX()*0.001-40);
            G4double y0 =   (G4UniformRand()-0.5)*(pCTXMLInput->GetPlaneRows()*pCTXMLInput->GetPixelY()*0.001-40);
